package com.example.picturebackend.manage.sharding;
...
public class DynamicShardingManager {
    ...
    private static final String DATA_SOURCE_NAME = "picture_db"; // 配置文件中的数据源名称（actual-data-nodes 前缀）
    ...
    private Set<String> fetchAllPictureTableNames() {
        ...
        Set<Long> spaceIds = spaceMapper.selectList(queryWrapper)
                .stream()
                .map(Space::getId)
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> DATA_SOURCE_NAME + "." + tableName) // actual-data-nodes 前缀为数据源名称
        ...
        try (ShardingSphereConnection connection = dataSource.getConnection().unwrap(ShardingSphereConnection.class)) {
            ...
            throw new RuntimeException("更新动态分表配置失败", e);
        }
    }
